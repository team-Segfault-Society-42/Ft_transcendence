...
<img width="822" height="268" alt="image" src="https://github.com/user-attachments/assets/63f58f6d-cf3e-402d-911f-bbdab79ddc36" />
...
| --- | --- |
| ameechan | - Docker, Docker Compose, Makefile<br>- NGINX proxy config<br>- Infrastructure setup (separate `dev` and `prod` build targets)<br>- Database seeding |
| mbendidi | - Game logic, Tic-Tac-Toe engine<br>- WebSocket gateway (game + chat)<br>- Spectator mode<br>- Chat system |
| nryser | - JWT auth architecture (HttpOnly cookies, global guard, DTO validation, protected/public route logic)<br>- 2FA (TOTP): QR onboarding, partial auth flow with `2fa_pending` cookie, rate limiting, activate/deactivate 2FA<br>- 42 OAuth: account linking, username normalization and collision handling<br>- Friends system API (send, accept, decline, remove, list)<br>- Realtime presence and websocket authentication<br>- Realtime Friend events<br>- Profile and settings backend (avatar upload, public/private profile separation)<br>- Avatar upload validation/security<br>- Settings and friends page (frontend)<br>- Swagger/OpenAPI documentation<br>- Update user password and email<br>- I18n translations |
